...
```bash
# Téléchargez le dossier de travail via Google drive puis :
Téléchargez le zip via : https://drive.google.com/drive/folders/1V4xLmsmIMFhbZW0EBeJZVBTrHvJUQgSY?usp=drive_link
unzippez le fichier helpdesk.zip
...
1. Quelle est votre couverture finale (% statements / branches / functions) ? Pourquoi est-elle < 100% sur certains fichiers ?
...
1. Ça marche ? Pourquoi ? Quelles seraient les **3 mitigations** ? (taille du secret, rotation, alg, etc.)
...
1. Quels headers de sécurité manquent ? (CSP, X-Frame-Options, Strict-Transport-Security, X-Content-Type-Options). Proposez un middleware Next.js qui les ajoute (code à inclure dans le rapport).
...
Dans `RAPPORT.md`, rédigez en conclusion :
