...
Connexion via `az login` (navigateur). Souscription Azure for Students vérifiée avec `az account list`.

## 6.2 Ressources Azure créées

| Ressource | Nom | Détail |
|---|---|---|
| Resource Group | `helpdesk-rg` | Région `francecentral` |
| Container Registry | `helpdeskacrav` | SKU Basic, admin activé |
| App Service Plan | `helpdesk-plan` | Tier B1, Linux |
| Web App | `helpdesk-av` | Image depuis ACR |

```bash
RG=helpdesk-rg
LOCATION=francecentral
ACR=helpdeskacrav
APP_NAME=helpdesk-av
PLAN=helpdesk-plan
...
## 6.3 Image poussée sur ACR

L'image Docker a été buildée avec un entrypoint qui exécute automatiquement `prisma migrate deploy` et le seed au premier démarrage, puis lance `node server.js`.

```bash
docker build -t helpdesk:dev .
docker tag helpdesk:dev helpdeskacrav.azurecr.io/helpdesk:latest
docker push helpdeskacrav.azurecr.io/helpdesk:latest
```

Taille de l'image finale : **279 MB** (< 300 MB).

## 6.4 Web App configurée

```bash
az webapp config container set \
  --name helpdesk-av --resource-group helpdesk-rg \
  --docker-custom-image-name helpdeskacrav.azurecr.io/helpdesk:latest \
  --docker-registry-server-url https://helpdeskacrav.azurecr.io \
  --docker-registry-server-user helpdeskacrav \
  --docker-registry-server-password "$ACR_PASS"

az webapp config appsettings set --resource-group helpdesk-rg --name helpdesk-av \
  --settings \
    DATABASE_URL="file:/home/prod.db" \
    JWT_SECRET="<secret-aléatoire-32-bytes>" \
    NODE_ENV="production" \
    WEBSITES_PORT=3000 \
    WEBSITES_ENABLE_APP_SERVICE_STORAGE=true
```

`WEBSITES_ENABLE_APP_SERVICE_STORAGE=true` est indispensable pour que `/home/prod.db` (SQLite) soit persisté entre les redémarrages du conteneur.

## 6.5 Initialisation de la DB en prod

L'initialisation est automatique grâce à l'`entrypoint.sh` inclus dans l'image :
...
Au premier démarrage, le conteneur exécute les migrations Prisma puis le seed (création des utilisateurs admin/user et des tickets de démo). Le fichier `/home/.seeded` (sur le volume persistant `/home`) empêche de re-seeder lors des redémarrages suivants.

## 6.6 Vérification du déploiement

```bash
curl https://helpdesk-av.azurewebsites.net/api/health
# → {"status":"ok","timestamp":"...","uptime":...}
```

URL publique : **https://helpdesk-av.azurewebsites.net**

Connexion avec `admin@helpdesk.io / Password123!` fonctionnelle.

## 6.7 CI/CD connecté au déploiement

La subscription Azure for Students ne permet pas de créer des service principals (`az ad sp create-for-rbac` → erreur de permission AD). Solution retenue : **ACR Webhook + Continuous Deployment**.

1. Activation du continuous deployment sur la Web App :
   ```bash
   az webapp deployment container config --enable-cd true \
     --name helpdesk-av --resource-group helpdesk-rg
   ```
2. Création d'un webhook ACR qui déclenche le redéploiement quand `:latest` est poussé :
   ```bash
   az acr webhook create --name helpdeskavwebhook \
     --registry helpdeskacrav --actions push \
     --scope "helpdesk:latest" --uri "<WEBHOOK_URL>"
   ```
3. Le job `deploy` du workflow GitHub Actions pousse l'image sur ACR (avec les secrets `ACR_LOGIN_SERVER`, `ACR_USERNAME`, `ACR_PASSWORD`), le webhook déclenche automatiquement le redéploiement.

Secrets GitHub configurés :

| Secret | Valeur |
|---|---|
| `ACR_LOGIN_SERVER` | `helpdeskacrav.azurecr.io` |
| `ACR_USERNAME` | `helpdeskacrav` |
